...
adv :: (Typeable1Monad m) => Advice m Int Int
adv proceed n = proceed (n+2)

adv2 :: (Typeable1Monad m) => Advice m Int Int
adv2 proceed n = proceed 32

adv3 :: (Typeable1Monad m) => Advice m Int Int
adv3 proceed n = proceed (n-100)
...
successor :: Typeable1Monad m => Int -> m Int
successor n = return (n+1)

successor2 :: Typeable1Monad m => Int -> AOT m Int
successor2 n = successor # n

successor3 :: Monad m => Int -> m Int
successor3 n = return (n+1)

successor'' :: Int -> AOID Int
successor'' n = return (n+1)

successor2'' :: Int -> AOID Int
successor2'' n = successor'' # n

client1 n = runAOID (program n)
program :: Int -> AOID Int
...
program3 :: Int -> AOID Int
program3 n = let s  = successor''  -- :: Int -> AOID Int
                 s2 = successor2'' -- :: Int -> AOID Int
             in do 
...
program5 :: Int -> AOID Int
program5 n = let s = successor 
             in do
                x <- s # n -- x = 4
                let a = aspect (pcCall s) (adv::Advice AOID Int Int)
                deploy a
                y <- s # n -- y = 6
                undeploy a
                z <- s # n -- z = 1
...
program6 n = do -- we specify the type signature using undefined :: sig. We could also define a macro in Template Haskell.
                ...
                deploy (aspect (pcType (undefined :: Int -> AOID a)) adv6)
                x <- successor # n
                y <- successor2 # n
                return (x+y)
-- Tests
...
program7 :: Int -> AOID Int
program7 n = let s  = successor
                 s2 = successor2
             in do -- all jps with the type, but pcCalls to successor2
                   deploy (aspect (pcType (undefined :: Int -> AOID Int) `pcAnd` pcNot (pcCall s2)) adv)
                   x <- s # n
                   y <- s2 # n
                   return (x+y)
-- Tests
...
typprogram1 :: Int -> AOID Int
typprogram1 n = do
                 deploy (aspect (pcType (undefined :: Int -> AOID Int)) adv)
...
typprogram2 :: Int -> AOID Int
typprogram2 n = do
                  -- the generic advice cannot do much. We make it print something.
                  deploy (aspect (pcType (undefined :: Int -> AOID Int)) genericAdvice)
...
concreteAdvice :: Advice AOID [Int] Int
concreteAdvice proceed arg = proceed ([42] ++ arg)

-- not passing tests, maybe problem with pcCall pc
poly1 xs = runAOID (polyProgram1 xs)
polyProgram1 :: [Int] -> AOID Int
polyProgram1 xs = do 
                     deploy (aspect (pcCall (polyHead :: [Int] -> AOID Int)) concreteAdvice)
                     x <- polyHead # ([1, 2, 3] :: [Float])
                     polyHead # xs
-- Tests
prop_poly1 xs = poly1 xs == (42 :: Int)
...
polyProgram2' :: Int -> AOID Int
polyProgram2' n = do
                   deploy (aspect (pcType (undefined :: Int -> AOID Int)) polyAdvice')
...
polyProgram5' :: [Int] -> AOID Int
polyProgram5' xs = do
                     deploy (aspect (pcType (undefined :: [a] -> AOID a)) polyAdvice3')
                     polyHead2 # xs
-- Tests
prop_poly5' xs = not (null xs) ==> poly5' xs == last xs where types = xs :: [Int]
...
polyProgram6 xs = let f = polyHead2 :: [Int] -> AOID Int
