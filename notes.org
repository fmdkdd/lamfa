...
The instrumentation is then effortless.  First we define the
facet-specific rules by extending the basic rules objects (=append
... *rules*=).
...
Note the reference to the free dynamic variable =*pc*=.  Then in the
entry point to facet evaluation, we override the standard rules with
the new ones.  We also declare the =*pc*= argument to be dynamically
scoped inside this call, using =(declare (special *pc*))=.
