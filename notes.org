...
#+BEGIN_SRC js
  ...
  let rules = {
    constant: function(σ, θ, {c}) {
      return [ σ, c ];
    },

    variable: function(σ, θ, {x}) {
      return [ σ, θ[x] ];
    },

    abstraction: function(σ, θ, {x, e}) {
      return [ σ, [x, e, θ] ];
    },

    application: function(σ, θ, {e1, e2}) {
    ...
    dereference: function(σ, θ, {e}) {
      let [σ1, a] = ↆ(σ, θ, e);
      return [ σ1, σ1[a] ];
    },

    assignment: function(σ, θ, {e1, e2}) {
      let [σ1, a] = ↆ(σ, θ, e1);
      let [σ2, v] = ↆ(σ1, θ, e2);
...
We cannot show /every/ way to write such an interpreter in JavaScript.
Here we tried to be syntaxically as close as possible to the original
big-step semantics, using ES6 destructuring and Unicode identifiers.
Of course, destructuring is just syntactic sugar, so we could unsugar
our version to basic features of the language only.  The main
mechanism used here is the ad-hoc pattern matching achieved by
dispatching on the dynamic type of the AST node or interpreter value
(by using an explicit ‘type’ property).

Another, maybe more familiar way to write such an interpreter in
JavaScript is to use inheritance.

#+BEGIN_SRC js

#+END_SRC

In any case, we can easily instrument this base interpreter by
following the operational semantics from Austin and Flanagan.
...
#+BEGIN_SRC js
  ...
  let rules = {
    constant: function(σ, θ, pc, {c}) {
      return [ σ, c ];
    },

    variable: function(σ, θ, pc, {x}) {
      return [ σ, θ[x] ];
    },

    abstraction: function(σ, θ, pc, {x, e}) {
      return [ σ, {type: 'closure', x, e, θ} ];
    },

    application: function(σ, θ, pc, {e1, e2}) {
    ...
    dereference: function(σ, θ, pc, {e}) {
      let [σ1, v] = ↆ(σ, θ, pc, e);
      return [ σ1, deref(σ1, v, pc) ];
    },

    assignment: function(σ, θ, pc, {e1, e2}) {
      let [σ1, v1] = ↆ(σ, θ, pc, e1);
      let [σ2, v2] = ↆ(σ1, θ, pc, e2);
      return [ assign(σ2, pc, v1, v2), v2 ];
    },
  };
  ...
  interpretProgram({
    ...
    e2: {
      type: 'reference',
      e: {type: 'constant', c: 42}
    }
  },
  {},{},[1]);
#+END_SRC

# Same issues appear, so it’s not only Narcissus.  Maybe it’s JS!
