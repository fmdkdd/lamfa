...
[[file:js/lab/oo-std-facets.diff]]
#+BEGIN_SRC diff
0a1,53
> Set.prototype.union = function(elem) {
>   let n = new Set(this);
...
1. We can use a global variable for the ‘pc’.  This eliminates the
   ...
   This temporary rewriting of a variable is essentially simulating a
   dynamic binding of the ‘pc’ variable.  There are no built-ins
   constructs or syntactic sugar for dynamic binding in JavaScript,
   but we can add one for this particular variable.
...
2. Using a global variable for just the ‘pc’ and not the store or
   ...
   =C= is opaque: we do not see what the context made is of by looking
   at the formal parameters.  Only looking at the entry point
