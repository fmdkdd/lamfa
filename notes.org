...
pattern we would like to abstract one and for all.
...
** Haskell
Dynamic scoping is far from common in today’s mainstream languages.
It brings additional complexities in thread-safety and tail call
optimization.  Even without proper dynamic scoping, we can emulate it
if the language allows mutability of variables.  This begs the
question: what tools do we have in a pure, immutable language without
dynamic scoping to deal with the problem of modular instrumentation?
