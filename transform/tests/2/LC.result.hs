...
apply f v = fail (show f ++ show v)
--apply (ValueR c l) v = apply c v >>= \ r -> return $ ValueR r l
...
-- s1 = [("a",Plus),("b",Minus),("m",Plus),("n",Plus),("t",Minus),("u",Minus)]
-- s2 = [("m",Plus),("n",Minus),("t",Minus),("u",Plus)]
-- s3 = [("a",Plus),("m",Plus),("n",Minus)]

-- s3' = let p = filter plusTag s1
--           n = filter minusTag s1
--           r = deleteFirstsBy tagEq s2 n
