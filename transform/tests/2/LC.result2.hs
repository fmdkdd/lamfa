...
instance Show TraceData where
        show (Allow l1 l2 b)
          = "Allow " ++ showSend l1 ++ " " ++ showReceive l2 ++ " -> " ++
              show b
        show (Propagate l1 l2 l3)
          = "Propagate " ++ showSend l1 ++ " " ++ showSend l2 ++ " -> " ++
              showSend l3
        show (Return v) = "R := " ++ show v
        show (Call c f v)
          = "Apply f:" ++ show f ++ " to v:" ++ show v ++ " caller:" ++
              show c
        show (Str s) = s
...
allowCheck ::
           (MonadWriter TraceR m) => Int -> Labels -> Labels -> m ()
allowCheck step l1 l2
  = tell [Line step $ Allow (addDefault l1) (addDefault l2) allowed]
...
printTrace [] _ = return ()
printTrace ((Line i s) : ts) n
  = do putStrLn $ (nspaces n) ++ show i ++ ". " ++ show s
       printTrace ts n
