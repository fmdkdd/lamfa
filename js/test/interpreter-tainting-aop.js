...
var interpreter = require('../lib/interpreter-tainting-aop')
...
function testInterpretTaint(name, AST, expectedValue, expectedStore, options) {
  test(name, function() {
    interpreter.activateTainting();
    var result = interpret(AST, options);
    should.exist(result, 'Undefined result object');
    should.exist(result.value, 'Undefined result value');
    should.exist(result.store, 'Undefined store');
    result.value.should.eql(expectedValue);
    result.store.elements.should.eql(expectedStore);
    interpreter.deactivateTainting();
  });
}
...
suite('interpret (tainted values) AST node', function() {
  testInterpretTaint('taintExpr',
                nodes.taintExpr(nodes.constant(42)),
                new TaintedValue(42), []);

  testInterpretTaint('taintExpr with PC set',
                nodes.taintExpr(nodes.constant(42)),
                new TaintedValue(42), [], {pc: [1]});

  testInterpretTaint('taintExpr with negative PC',
                nodes.taintExpr(nodes.constant(42)),
                new TaintedValue(42), [], {pc: [-1]});

  testInterpretTaint('dereference a tainted value',
                nodes.dereference(
                  nodes.taintExpr(nodes.reference(nodes.constant(42)))),
                new TaintedValue(42),
                [42]);

  testInterpretTaint('assign to a tainted value',
               nodes.assignment(
  ...
  var λ2 = nodes.abstraction(nodes.variable('x'),
                             nodes.constant(2));
  testInterpretTaint('apply a tainted function',
                nodes.application(
                  nodes.taintExpr(λ1),
                  nodes.constant(0)),
                new TaintedValue(0), []);
  ...
  testInterpretTaint('if <true> then 2 else 3',
