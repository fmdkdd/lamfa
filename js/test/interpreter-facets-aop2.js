...
var interpreter = require('../lib/interpreter-facets-aop2');
...
function testInterpretFacet(name, AST, expectedValue, expectedStore, options) {
  test(name, function() {
...
suite('interpret (facets) AST node', function() {
  testInterpretFacet('facetExpr',
                nodes.facetExpr(nodes.constant(1),
                                nodes.constant(42),
                                nodes.bottom()),
                createFacet(1, 42, bottom), []);

  testInterpretFacet('facetExpr with PC set',
                nodes.facetExpr(nodes.constant(1),
                                nodes.constant(42),
                                nodes.bottom()),
                42, [], {pc: [1]});

  testInterpretFacet('facetExpr with negative PC',
                nodes.facetExpr(nodes.constant(1),
                                nodes.constant(42),
                                nodes.bottom()),
                bottom, [], {pc: [-1]});

  testInterpretFacet('dereference a facet',
                nodes.dereference(
                  nodes.facetExpr(nodes.constant(1),
  ...
  testInterpretFacet('assign to a facet',
                     nodes.assignment(
                       nodes.facetExpr(nodes.constant(1),
                                       nodes.bottom(),
  ...
  var λ2 = nodes.abstraction(nodes.variable('x'),
                             nodes.constant(2));
  testInterpretFacet('apply a facet',
                     nodes.application(
