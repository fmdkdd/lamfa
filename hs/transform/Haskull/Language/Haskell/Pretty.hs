...
module Haskull.Language.Haskell.Pretty (
                -- * Pretty printing
                Pretty,
...
instance Pretty HsDecl where
        ...
        pretty (HsOpenDataDecl loc context name nameList constrList derives) =
                blankline $
                markLine loc $
                mySep ( [text "opendata", ppHsContext context, pretty name]
                        ++ map pretty nameList)
                        <+> (myVcat (zipWith (<+>) (equals : repeat (char '|'))
                                                   (map pretty constrList))
                        $$$ ppHsDeriving derives)

        pretty (HsExtendDataDecl loc context name nameList constrList derives) =
                blankline $
                markLine loc $
                mySep ( [text "extenddata", ppHsContext context, pretty name]
                        ++ map pretty nameList)
                        <+> (myVcat (zipWith (<+>) (equals : repeat (char '|'))
                                                   (map pretty constrList))
                        $$$ ppHsDeriving derives)
        ...
        pretty (HsExtendDecl pos name declList) =
                blankline $
                markLine pos $
                mySep ( [text "extend", pretty name]
                        ++ [text "where"])
                $$$ ppBody classIndent (map pretty declList)
